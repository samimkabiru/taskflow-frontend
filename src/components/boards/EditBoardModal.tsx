...
export default function EditBoardModal({ board, open, onClose, onSaved }: EditBoardModalProps) {
  const [name, setName] = useState(board.name);
  const [description, setDescription] = useState(board.description);
  const [accentColor, setAccentColor] = useState(board.accentColor || ACCENT_COLORS[0]);
  const [taskPrefix, setTaskPrefix] = useState(board.taskPrefix || "");
  ...
  const isPrefixValid = !taskPrefix || (taskPrefix.length >= 2 && taskPrefix.length <= 6 && /^[A-Z]{2,6}$/.test(taskPrefix));
  const showPrefixError = prefixTouched && taskPrefix.length > 0 && !isPrefixValid;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updated = await updateBoard(board.id, {
        ...
        accentColor,
        taskPrefix: taskPrefix.trim() || board.taskPrefix,
      });
      ...
      const errorMsg = err instanceof Error ? err.message : "Failed to update board";
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6 pt-4 space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-surface-lowest border border-outline-variant rounded-lg px-3 py-2.5 font-[family-name:var(--font-body)] text-[14px] text-on-surface focus:outline-none focus:border-primary-container focus:ring-1 focus:ring-primary-container transition-colors"
              required
              autoFocus
            />
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <span className="font-[family-name:var(--font-mono)] text-[11px] text-primary font-medium">
                Preview: {taskPrefix ? `${taskPrefix}-1` : board.taskPrefix ? `${board.taskPrefix}-1` : "TSK-1"}
              </span>
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-surface-lowest border border-outline-variant rounded-lg px-3 py-2.5 font-[family-name:var(--font-body)] text-[14px] text-on-surface focus:outline-none focus:border-primary-container focus:ring-1 focus:ring-primary-container transition-colors resize-none"
            />
          </div>
